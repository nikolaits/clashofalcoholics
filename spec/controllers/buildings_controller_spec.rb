...
RSpec.describe BuildingsController, type: :controller do
  describe 'POST /buildings/upgrade' do
    before(:each) do
      @district = create(:district, beer: 100, food: 100, vodka: 100, stone: 0)
      @usr = @district.user
    ...
    it "returns an error if user doesn't have enough rosources for upgrade" do
      @district.vodka = 0
      @district.save
      create(:building_level, building_id: @building.id, level: 1, vodka: 10, food: 10, beer: 10)

      post :upgrade, building_id: @building.id, format: 'json'
      error = assigns(:error)
      expect(error).to eq(true)
    end

    it 'returns an error if maximum level reached' do
      create(:district_building, district_id: @district.id, level: 1)
      create(:building_level, building_id: @building.id, level: 1, vodka: 10, food: 10, beer: 10)
    ...
    it 'creates a new building record' do
      create(:building_level, building_id: @building.id, level: 1, vodka: 10, food: 10, beer: 10, stone: 0)

      post :upgrade, building_id: @building.id, format: 'json'

      expect(DistrictBuilding.where(district_id: @district.id, building_id: @building.id).first.level).to eq(1)
    end

    it 'increments building level of existing building' do
      create(:district_building, district_id: @district.id, building_id: @building.id, level: 1)
      create(:building_level, building_id: @building.id, level: 2, vodka: 10, food: 10, beer: 10, stone: 0)

      post :upgrade, building_id: @building.id, format: 'json'

      expect(DistrictBuilding.where(district_id: @district.id, building_id: @building.id).first.level).to eq(2)
    end
